...
from rect_utils import union_all
...
class _NodeCursor(object):
    ...
    def query_rect(self, rect):
        def p(o, x):
            return rect.is_intersect(o.rect)
    ...
    def query_point(self, point):
        def p(o, x):
            return o.rect.is_containpoint(point)
    ...
    def _become(self, index):
        ...
        x = self.rpool[recti]
        y = self.rpool[recti + 1]
        xx = self.rpool[recti + 2]
        yy = self.rpool[recti + 3]
    ...
    def _save_back(self):
        ...
        if self.rect is not NullRect:
            self.rect.write_raw_coords(self.rpool, recti)
        else:
            self.rpool[recti] = 0
            self.rpool[recti + 1] = 0
            self.rpool[recti + 2] = 0
            self.rpool[recti + 3] = 0
